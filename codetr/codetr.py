...
class CoDETR(BaseDetector):
    ...
    def __init__(
        ...
        if rpn_head is not None:
            rpn_train_cfg = (
                train_cfg[head_idx].rpn if (train_cfg is not None and train_cfg[head_idx] is not None) else None
            )
        ...
        for i in range(len(roi_head)):
            if roi_head[i]:
                rcnn_train_cfg = (
                    train_cfg[i + head_idx].rcnn if (train_cfg and train_cfg[i + head_idx] is not None) else None
                )
